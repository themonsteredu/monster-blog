...
#
# 흐름: 글 만들기 → 이미지·썸네일 → 네이버 올리기(임시저장) → 사람이 발행
...
with tab_set:
    ...
    o_key = st.text_input("OpenAI(ChatGPT) API 키 — 그림", value=settings["openai_api_key"], type="password")
    st.divider()
...
with tab_write:
    ...
    style = st.radio("이미지 스타일", ["일러스트", "실사"], horizontal=True)
...
with tab_image:
    if not st.session_state.get("post_text"):
        ...
        if not prompts:
            st.write("글에서 이미지 설명을 찾지 못했어요. (전용틀의 '이미지 그림 설명' 형식을 확인)")
        if st.button("🖼️ 이미지 생성", type="primary"):
            if not settings["openai_api_key"]:
                st.error("설정 탭에서 OpenAI 키를 먼저 저장하세요.")
            else:
                paths = {}
