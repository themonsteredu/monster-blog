...
4) 마무리 한두 문장(자연스럽게. 전화번호·카카오톡 주소·상담 안내는 여기 쓰지 말 것 — 맨 아래 학원 소개에 이미 있어 중복됨)
분량 약 1,500자. [인용]과 [이미지N]은 각각 그 줄에 단독으로 둘 것.

[이미지 설명] 본문에 넣은 [이미지N] 개수만큼, 글 맨 끝에 "이미지N: 설명" 형식으로 한 줄씩 쓸 것.
각 설명은 서로 완전히 다른 장면·소재·구도로 (예: 교실 전경 / 문제집 위의 손 클로즈업 / 칠판 앞 뒷모습 / 상담 테이블).
같은 소재(예: 책)를 반복하지 말고, 그 이미지가 들어가는 문단 내용과 어울리게.
...
[최종 출력 순서]
제목 / 본문(도입→본문+[이미지N]→마무리) / 학원 소개 / 이미지 설명(이미지N: 한 줄씩) / 해시태그(# 붙여 10~15개)`;
